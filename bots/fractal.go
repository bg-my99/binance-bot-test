...
func (f *FractalBot) AddMarketTrade(trade *binance.AggTrade) {
	...
	if ok := f.candles[len(f.candles)-1].AddTrade(trade); !ok {
		...
		if len(f.jawSMA5) < 5 {
			...
			f.jawLine.PointsChannel <- plotter.XY{X: f.candles[len(f.candles)-1].Timestamp, Y: f.runningJawSMA5}
		}
		...
		if len(f.teethSMA8) < 8 {
			...
			f.teethLine.PointsChannel <- plotter.XY{X: f.candles[len(f.candles)-1].Timestamp, Y: f.runningTeethSMA8}
		}
		...
		if len(f.lipsSMA13) < 13 {
			...
			f.lipsLine.PointsChannel <- plotter.XY{X: f.candles[len(f.candles)-1].Timestamp, Y: f.runningLipsSMA13}
		}
